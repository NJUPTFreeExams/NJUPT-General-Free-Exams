---
title: UploadGuide
description: 上传指南。本篇主要介绍如何向本项目提交（PR）相关资料，以及相关注意事项。
---
...
:::note
因为在`/public/`文件夹下文件都不会自动构建网页。所以如果你想要上传md文件，建议在`src/content/docs/reserve`文件夹下复制一份，并加上frontmatter
```yaml "（可选）"
---
title: 文件标题
description: 内容描述（可选）
...
:::caution
该项目在增加前端界面后，文件目录结构发生了很大变化，请将上传的资料统一放置在`public`文件夹下对应的课程目录下。

如果需要新开一门课程，请在`public`文件夹下新建一个文件夹，并在`src/content/docs`中对应开课学院的文件夹下新建一个md文件，要求md文件的frontmatter的**title与新建的文件夹的名称保持完全一致**。
:::

### 如有需要，可为子文件夹创建 README 并作说明

这里有一份 README.md 文件[`样例`](https://github.com/NJUPTFreeExams/njuptfreeexams.github.io/blob/master/sample.md)。
